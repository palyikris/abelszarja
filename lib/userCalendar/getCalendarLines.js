export function GetCalendarLines(list) {
  ...
  list.map((dayData, j) => {
    for (let i = 0; i < 15; i++) {
      ...
      if (subject) {
        if (
          subject.timeStart.split(":")[0] === "7" ||
          subject.timeStart.split(":")[0] === "07"
        ) {
          seven[j] = subject;
        } else if (
          subject.timeStart.split(":")[0] === "8" ||
          subject.timeStart.split(":")[0] === "08"
        ) {
          eight[j] = subject;
        } else if (
          subject.timeStart.split(":")[0] === "9" ||
          subject.timeStart.split(":")[0] === "09"
        ) {
          nine[j] = subject;
